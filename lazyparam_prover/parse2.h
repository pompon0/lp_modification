...
struct ParseCtx {
  ...
  void parse_file(tptp::File *file, std::istream &is) { FRAME("parse_file");
    ...
    for(auto f : funs) {
      str name = Lib::env.signature->functionName(f).c_str();
      info("% -> %",f,name);
    }
    for(auto p : preds) {
      str name = Lib::env.signature->predicateName(p).c_str();
      info("% -> %",p,name);
    }
  }
};
