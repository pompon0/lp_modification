...
List<Cont> strong(StrongFrame f) const { FRAME("strong(%,%)",show(f->dcla),f->strong_id);
  ...
  List<Atom> todo;
  for(ssize_t i=cla.atom_count(); i--;) if(i!=f->strong_id) todo.push(state->A,cla.atom(i));
  auto matoms = strong_resolution(f->nodes_limit,todo);
  if(!matoms) return nothing();
  
  WeakConnectionsFrame::Builder b(state->A);
...
Maybe<List<Atom>> strong_resolution(size_t nodes_limit, List<Atom> todo) const { FRAME("strong_resolution()");
  List<Atom> checked;
  while(!todo.empty()) {
    auto a = todo.head();
    todo = todo.tail();
    // Look for strong only atoms.
    if(!a.strong_only()) { checked.push(state->A,a); continue; }
    ...
    if(filter.next()) { checked.push(state->A,a); continue; }
    ...
    for(size_t i=cla.atom_count(); i--;) if(i!=ca.i) todo.push(state->A,cla.atom(i));
  }
  return just(checked);
}
