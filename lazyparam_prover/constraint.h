...
struct OrderAtom {
  ...
  private:
    u8 *ptr;
  public:
    INL Builder(Relation rel, size_t pair_count) : ptr(TERM_PAIRS::alloc(pair_count)) {
  ...
  // ignores sign
  INL static OrderAtom neq(Atom l, Atom r) {
  ...
  INL OrderAtom(Relation rel, Term l, Term r) : OrderAtom(Builder(rel,1).set_pair(0,{l,r}).build()) {}
