...
List<Cont> lazy_pre_strong_connection(LazyPreStrongConnectionFrame f) const { 
  ...
  for(size_t i=cla.atom_count(); i--;) if(i!=f->strong_id) bs.push(state->A,cla.atom(i));
  ...
  for(auto apl = paths(state->A,L); !apl.empty(); apl = apl.tail()) {
    ...
    LazyStrongConnectionFrame::Builder b1(state->A),b2(state->A);
