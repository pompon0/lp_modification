...
List<Cont> weak_set(WeakSetFrame f) const { STATE_FRAME(state,"weak_set");
  ...
  wsb->branch_count = f->branch_count-1;
  wsb->branches = f->branches.tail();
  alts.push(state->A,builder().add(Frame(wsb.build())).add(Frame(wb.build())).build());
  return alts;
}
